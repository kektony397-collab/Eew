...
const SettingsView: React.FC = () => {
    ...
    const [localAverage, setLocalAverage] = useState(vehicleAverageKmL);
    const [localTankSize, setLocalTankSize] = useState(tankSize);
    ...
    useEffect(() => {
        setLocalAverage(vehicleAverageKmL);
        setLocalTankSize(tankSize);
    }, [vehicleAverageKmL, tankSize]);

    const handleSaveSettings = async () => {
        setVehicleAverage(localAverage);
        setTankSize(localTankSize);
        await db.settings.put({ key: 'vehicleAverageKmL', value: localAverage });
        await db.settings.put({ key: 'tankSize', value: localTankSize });
    ...
    const InputField = ({ label, value, onChange, type = 'number', unit }: { label: string, value: number | string, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, type?: string, unit?: string }) => (
        <div className="mb-4">
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            <Card>
                ...
                <InputField label="Vehicle Average" value={localAverage} onChange={(e) => setLocalAverage(parseFloat(e.target.value))} unit="km/L" />
                <InputField label="Fuel Tank Size" value={localTankSize} onChange={(e) => setLocalTankSize(parseFloat(e.target.value))} unit="Litres" />
            ...
            <Card>
                ...
                <InputField label="Amount" value={calcAmount} onChange={(e) => setCalcAmount(e.target.value)} unit="INR" />
                <InputField label="Price per Litre" value={calcPrice} onChange={(e) => setCalcPrice(e.target.value)} unit="INR" />
